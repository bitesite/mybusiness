...
const BlogPost = ({ blogPostId }) => {
  ...
  const getBlogPost = () => {
    $.getJSON(`/blog/${blogPostId}`, (data) => {
      const { title, body: text, author, published_at: publishedAt, featured_image: featuredImage, tag_list: tags      } = data.blog_post;
  ...
  return (
    ...
    <StyledBlogPost className="blog-post-component-mount-point" vertical alignItems="center" justifyContent="center" gap={isMobileWidth ? 40 : 60}>
      {blogPost && (
        <>
       {blogPost.featuredImage && (
        <ImageContainer>
          <img src={blogPost.featuredImage.url} alt="Blog Post about the article" />
        </ImageContainer>
        )}
        <BlogPostHeader vertical alignItems="flex-start" justifyContent="center" gap={20}>
          ...
          <div className="blog-post-title heading-regular">{blogPost.title}</div>
          <Frame alignItems="center" justifyContent="flex-start" gap={8} className="blog-post-author" width="100%">
            {blogPost.author.avatar && blogPost.author.avatar.url ? (
              <Avatar src={blogPost.author.avatar_url} alt="Author Avatar" />
            ) : (
            ...
            <Frame alignItems="center" justifyContent="center" gap={8}>
              ...
              <div className="caption-light">{moment(blogPost.publishedAt).format('MMMM D, YYYY')}</div>
            </Frame>
          </Frame>
        </BlogPostHeader>
        <BlogPostBody alignItems="center" justifyContent="center" gap={8} className="blog-post-content">
        ...
        {relatedPosts && relatedPosts.length > 0 && (
          <Frame className="related-posts-container" vertical alignItems="center" justifyContent="center" gap={isMobileWidth ? 40 : 60}>
             <div className="related-post-title heading-regular">Related Posts</div>
            <RelatedPosts alignItems="center" justifyContent="center" gap={20}>
              {relatedPosts.map((post, i) => (
                <BlogCard key={i} blogPost={post} tags={getTagList(post.tag_list)} className="related-post"  onClick={() => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (element) {
    ReactDOM.render(<BlogPost blogPostId={element.dataset.blogPostId} />, element);
  }
});
